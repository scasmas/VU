...
void printQueue(struct Node *start, bool positive, FILE *rp)
{
    struct Node *elem;
    elem = start;
    if(positive)
    {
        ...
        {
            if(elem -> data > 0)
                fprintf(rp, "L%d", elem -> data);
            elem = elem -> next;
            if(elem != NULL && elem -> data > 0)
                fprintf(rp,", ");
        }
    }
    if(!positive)
...
int giveFirstOfGivenPrio(struct Node *start, int prio)
{
    struct Node *elem;
    elem = start;
    while (elem != NULL)
